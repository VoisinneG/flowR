...
plotGatingSet2Input <- function(id, simple_plot = TRUE, auto_update = TRUE) {
...
if (interactive()){
  ...
  ui <- fluidPage(
    fluidRow(
      column(4, plotGatingSet2Input("module")),
  ...
  server <- function(input, output, session) {
    ...
    observe({
      ...
      plot_params$samples <- pData(gs)$name[2]
    })
